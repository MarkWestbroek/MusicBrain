...
function slider(id, label, x, y, opts = {}) {
    return {
        control: {
            kind: 'slider', id, label,
...
function elements() {
    ...
    const cols = [w * 0.07, w * 0.20, w * 0.31, w * 0.42, w * 0.55, w * 0.67, w * 0.82, w * 0.94];
    const topY = 22;
    const bigY = 50;
    const lowKnobY = 78;
    const attenY = 96;
...
function mmbAhdsr() {
    ...
    const colX = [w * 0.12, w * 0.30, w * 0.50, w * 0.70, w * 0.88];
    return assemble({
        typeId: 'tp_mmb_ahdsr',
        categoryId: 'envelope',
...
function mmbSeq8() {
    ...
    const rowY1 = 38;
    const rowY2 = 62;
    const sx = (i) => w * (0.07 + (i % 8) * 0.115);
    const rowY = (i) => (i < 8 ? rowY1 : rowY2);
    // LED-rij iets onder elke knop.
    const ledY = (i) => rowY(i) + 9;
    const stepKnobs = [];
    const stepLeds = [];
    const stepDefaults = [0, 4, 7, 12, 7, 0, 5, 3, 12, 9, 7, 5, 4, 0, -5, -12];
    for (let i = 0; i < 16; i++) {
        const id = `s${i + 1}`;
        stepKnobs.push(knob(id, String(i + 1), sx(i), rowY(i), {
            size: 'small', min: -24, max: 24, def: stepDefaults[i] ?? 0, unit: 'st', color: '#f9fafb',
        }));
        stepLeds.push(led(`led_${id}`, sx(i), ledY(i), { color: '#fbbf24', size: 'small', bindTo: '__currentStep', bindMatch: i + 1 }));
    }
...
export function seedInternals(project) {
    ...
    const updatedRacks = racks.map((r) => r.id === internal.id

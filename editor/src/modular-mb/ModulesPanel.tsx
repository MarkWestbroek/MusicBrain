...
function defaultPortsFor(kind: ModuleKind): { inputs: Port[]; outputs: Port[]; params: ModuleDef['params'] } {
...
export function ModulesPanel(): JSX.Element {
  ...
  function addModule(kind: ModuleKind): void {
  ...
  function renameModule(id: string, label: string): void {
    updateProject((p) => ({
      ...p,
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 12, alignSelf: 'center', color: '#6b7280' }}>
          + Module:
        </span>
        {ALL_KINDS.map((k) => (
          <button key={k} onClick={() => addModule(k)} style={{ fontSize: 12 }}>
            {k}
          </button>
        ))}
      </div>
      ...
      {project.modules.length === 0 && (
        <p style={{ color: '#6b7280', fontSize: 13 }}>
          Nog geen modules. Voeg er een toe via de knoppen hierboven.
        </p>
      )}
          ...
          {project.modules.map((m) => (
            <tr key={m.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
              ...
              <td style={{ padding: '4px 8px', color: '#475569' }}>
                {m.inputs.map((p) => p.name).join(', ') || '—'}
              </td>
              <td style={{ padding: '4px 8px', color: '#475569' }}>
                {m.outputs.map((p) => p.name).join(', ') || '—'}
              </td>
              <td style={{ padding: '4px 8px', textAlign: 'right' }}>
                <button onClick={() => removeModule(m.id)} style={{ fontSize: 11 }}>×</button>
              </td>
            </tr>
          ))}
        </tbody>

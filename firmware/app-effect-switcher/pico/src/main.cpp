// main.cpp — MusicBrain effect-switcher firmware for RP2040 (Pico W).
//
// Responsibilities:
...
//   • Drive the relay hardware via the Relays driver (74HC595 chain).
//   • Listen for MIDI Program Change on both UART (DIN) and USB.
//
// REST API
// ────────
...
void handleStatus() {
  ...
  d["uptimeMs"] = millis();
  d["freeHeap"] = MB_FREE_HEAP();
  d["chip"]     = MB_CHIP_MODEL;
...
void setup() {
  ...
  // Wait for USB serial to enumerate (up to 3 seconds)
  ...
  Serial.println();
  Serial.println(F("=== MusicBrain effect-switcher starting ==="));
  Serial.printf("Version: %s (RP2040 Pico W)\n", APP_VERSION);
  ...
  // The WebServer doesn't do path parameters natively, so we use onNotFound

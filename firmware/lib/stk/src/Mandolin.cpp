...
#include "Mandolin.h"
#include "SKINImsg.h"
#include "MandolinData.h" // MMB: geembedde mand1..12.raw
...
Mandolin :: Mandolin( StkFloat lowestFrequency )
{
  ...
  // MMB: geen bestandssysteem op Teensy — de body-excitatiesamples zijn als
  // C-arrays geembed (MandolinData.h). De rate-init doet wat
  // FileWvIn::openFile deed: dataRate (22050) / huidige sample rate.
  for ( int i=0; i<12; i++ ) {
    soundfile_[i].setData( kMandolinBody[i].data, kMandolinBody[i].size, kMandolinBody[i].peak );
    soundfile_[i].setRate( 22050.0f / Stk::sampleRate() );
  }

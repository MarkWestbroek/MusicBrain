...
/**
 * @file cr78.h
 * @brief Eén berekende Roland CR-78-drum (kick, snare, rim, claves, cowbell,
 *        hihat, cymbal, maracas, guiro, bongo, conga, tamboerijn), float,
 *        header-only.
 * @details
 * Zelf-gemodelleerd naar de CompuRhythm CR-78 (1978): bridged-T-oscillatoren
 * (gedempte sinussen) voor de vellen, gefilterde ruis met envelope-VCA's voor
 * snare/hat/cymbal/maracas/guiro, en het accent-circuit dat harder én met meer
 * pitchbuiging aanslaat.
 *
 * Dit is de kernel die in `Cr78Module.h` (`tp_mmb_cr78`, FW-AU-16) in een
 * `AudioStream` zat, er letterlijk uit getild zodat de firmware en de
 * browser-simulator (`tools/mmb-wasm/cr78_wasm.cc`) dezelfde code draaien.
 * Alleen de sample rate van de coëfficiënten komt nu via `Init()` binnen;
 * bit-identiek geverifieerd met `tools/mmb-wasm/bitcheck/cr78_check.cc`.
 *
 * Gebruik: `Init(sr)`, instellen met de setters, `trigger()` op een stijgende
 * gate-flank, dan per sample `Tick()`. De uitkomst is in "drum-eenheden": de
 * Teensy-schil schaalt met 28000 naar int16, de wasm-schil met 28000/32768.
 */
#include <cmath>
#include <cstdint>
...
class Cr78 {
public:
    void Init(float sr) { sr_ = sr > 1.0f ? sr : 44100.0f; reconfigure(); }
    ...
    /** Eén sample in drum-eenheden (vóór de schaling naar de uitgang). */
    inline float Tick() {
...
private:
    ...
    float coef(float tauSec) const {
        return expf(-1.0f / (tauSec * sr_));
    }
    float noise() {   // LCG, ±1
    ...
    float hpCoef(float fcHz) const {
        return expf(-6.2832f * fcHz / sr_);
    }
    float lpCoef(float fcHz) const {
        return 1.0f - expf(-6.2832f * fcHz / sr_);
    }
    ...
    // Letterlijk overgenomen: de fase-increment rekent met een vaste 44,1 kHz,
    // de coëfficiënten met `sr_`. Op de Teensy en in de wasm zijn die gelijk.

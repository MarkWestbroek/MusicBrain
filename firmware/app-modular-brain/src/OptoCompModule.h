#pragma once
/**
 * @file OptoCompModule.h
 * @brief Opto-compressor in LA-2A-stijl (typeId `tp_mmb_opto_comp`, FW-FX-3 stap 2).
 *
 * @details
 * De DSP is mmb_dsp::OptoComp (dezelfde header draait als wasm in de
 * simulator); zie daar en doc/plans/vintage-compressors.md voor het karakter.
 * Stereo gekoppeld: één cel over L en R. Alleen `in_l` aangesloten = mono, en
 * dan komt het resultaat op beide uitgangen.
 *
 * Port map:
 ...
 * |-----------|--------------|---------|------------------------------------------|
 * | `peak`    | 0 … 100      | 40      | Peak Reduction: zakt de drempel naar −45 dBFS |
 * | `gain`    | -20 … +20 dB | 0       | uitgangsversterking                      |
 * | `mode`    | 0 / 1        | 0       | Compress (~3:1) of Limit                 |
 * | `color`   | 0 … 2        | 1       | buisverzadiging: 0 schoon, 2 dik         |
...
class OptoCompStream : public AudioStream {
public:
    OptoCompStream() : AudioStream(2, inputQueue_) { comp_.Init(AUDIO_SAMPLE_RATE_EXACT); }

    mmb_dsp::OptoComp& comp() { return comp_; }
...
private:
    ...
    mmb_dsp::OptoComp comp_;
    volatile float gr_ = 0.0f;
};

class OptoCompModule final : public AudioModule {
public:
    static constexpr const char* kTypeId = "tp_mmb_opto_comp";

    explicit OptoCompModule(std::string_view id) : AudioModule(kTypeId, id) {}

    AudioPort outputPort(std::string_view portId) const override {
        auto* s = const_cast<OptoCompStream*>(&stream_);
    ...
    void setControl(std::string_view controlId,
        ...
        auto& c = stream_.comp();
        if      (controlId == "peak")   c.set_peak_reduction(asFloat(40.0f));
        else if (controlId == "gain")   c.set_gain_db(asFloat(0.0f));
    ...
    static void registerFactory() {
        ...
        reg.register_(kTypeId,
            [](std::string_view id) -> std::unique_ptr<mb::runtime::Module> {
                return std::make_unique<OptoCompModule>(id);
            });
    }

private:
    mutable OptoCompStream stream_;
};

#pragma once
/**
 * @file ReverbModule.h
 * @brief Plaat-/veergalm, stereo (`tp_mmb_reverb`).
 *
 * mmb_dsp::Reverb: mode Plate (Dattorro-tank) of Spring (twee veren met
 * dispersieve all-pass-lussen). De vertragingspool (float, ~150 KB) komt
 * van de heap, zoals de band van het bandecho.
 *
 * Poorten: in_l/in_r (in = L) → out_l/out_r (out = L); CV size, mix (ook
 * `*_cv`). Controls: mode (0 plate / 1 spring), size, damp, predelay (ms),
 * mod, mix.
 */

#include "AudioModule.h"
#include "mb/runtime/Registry.h"
#include <Audio.h>
#include <cstdint>
#include <memory>
#include <new>
#include <string_view>
#include "mmb_dsp/reverb.h"

namespace mmb_link {

class ReverbStream : public AudioStream {
public:
    ReverbStream() : AudioStream(2, inputQueue_) {
        const int len = mmb_dsp::Reverb::poolLength(AUDIO_SAMPLE_RATE_EXACT);
        pool_.reset(new (std::nothrow) float[len]);
        if (pool_) rv_.Init(AUDIO_SAMPLE_RATE_EXACT, pool_.get(), len);
        else Serial.println("[reverb] pool alloc FAILED — module blijft droog");
    }
    mmb_dsp::Reverb& rv() { return rv_; }

    void update() override {
        audio_block_t* inL = receiveReadOnly(0);
        audio_block_t* inR = receiveReadOnly(1);
        audio_block_t* outL = allocate();
        audio_block_t* outR = allocate();
        if (!outL || !outR) {
            if (outL) release(outL); if (outR) release(outR);
            if (inL) release(inL);   if (inR) release(inR);
            return;
        }
        const audio_block_t* srcR = inR ? inR : inL;
        float x[2];
        for (int i = 0; i < AUDIO_BLOCK_SAMPLES; ++i) {
            x[0] = inL  ? inL->data[i]  * (1.0f / 32768.0f) : 0.0f;
            x[1] = srcR ? srcR->data[i] * (1.0f / 32768.0f) : 0.0f;
            rv_.Process(x);
            outL->data[i] = static_cast<int16_t>(clamp1(x[0]) * 32767.0f);
            outR->data[i] = static_cast<int16_t>(clamp1(x[1]) * 32767.0f);
        }
        transmit(outL, 0); transmit(outR, 1);
        release(outL); release(outR);
        if (inL) release(inL); if (inR) release(inR);
    }

private:
    static float clamp1(float v) { return v != v ? 0.0f : (v > 1.0f ? 1.0f : (v < -1.0f ? -1.0f : v)); }
    audio_block_t* inputQueue_[2] = { nullptr, nullptr };
    std::unique_ptr<float[]> pool_;
    mmb_dsp::Reverb rv_;
};

class ReverbModule final : public AudioModule {
public:
    static constexpr const char* kTypeId = "tp_mmb_reverb";
    explicit ReverbModule(std::string_view id) : AudioModule(kTypeId, id) {}

    AudioPort outputPort(std::string_view portId) const override {
        auto* s = const_cast<ReverbStream*>(&stream_);
        if (portId == "out_l" || portId == "out") return { s, 0, true };
        if (portId == "out_r") return { s, 1, true };
        return {};
    }
    AudioPort inputPort(std::string_view portId) const override {
        auto* s = const_cast<ReverbStream*>(&stream_);
        if (portId == "in_l" || portId == "in") return { s, 0, true };
        if (portId == "in_r") return { s, 1, true };
        return {};
    }
    PortKind inputPortKind(std::string_view portId) const override {
        if (portId == "in_l" || portId == "in_r" || portId == "in") return PortKind::Audio;
        if (cvPortIs(portId, "size") || cvPortIs(portId, "mix")) return PortKind::Cv;
        return PortKind::None;
    }
    PortKind outputPortKind(std::string_view portId) const override {
        if (portId == "out_l" || portId == "out_r" || portId == "out") return PortKind::Audio;
        return PortKind::None;
    }
    void writeCvPort(std::string_view portId, float value) override {
        auto& r = stream_.rv();
        if      (cvPortIs(portId, "size")) r.set_size(value);
        else if (cvPortIs(portId, "mix"))  r.set_mix(value);
    }

    void setControl(std::string_view controlId, mb::runtime::ControlValue value) override {
        auto asFloat = [&](float fb) -> float {
            if (auto* f = std::get_if<float>       (&value)) return *f;
            if (auto* i = std::get_if<std::int32_t>(&value)) return static_cast<float>(*i);
            if (auto* b = std::get_if<bool>        (&value)) return *b ? 1.0f : 0.0f;
            return fb;
        };
        auto& r = stream_.rv();
        if      (controlId == "mode")     r.set_mode(static_cast<int>(asFloat(0.0f) + 0.5f));
        else if (controlId == "size")     r.set_size(asFloat(0.6f));
        else if (controlId == "damp")     r.set_damp(asFloat(0.4f));
        else if (controlId == "predelay") r.set_predelay(asFloat(10.0f));
        else if (controlId == "mod")      r.set_mod(asFloat(0.3f));
        else if (controlId == "mix")      r.set_mix(asFloat(0.3f));
    }

    static void registerFactory() {
        auto& reg = mb::runtime::Registry::global();
        if (reg.has(kTypeId)) return;
        reg.register_(kTypeId, [](std::string_view id) -> std::unique_ptr<mb::runtime::Module> {
            return std::make_unique<ReverbModule>(id); });
    }

private:
    mutable ReverbStream stream_;
};

}  // namespace mmb_link

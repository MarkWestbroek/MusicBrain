#pragma once
/**
 * @file DiodeCompModule.h
 * @brief Diodebrug-compressor in Neve-33609-stijl (typeId `tp_mmb_diode_comp`, FW-FX-3 stap 6).
 *
 * @details
 * De DSP is mmb_dsp::DiodeComp (dezelfde header draait als wasm in de simulator);
 ...
 * |---|---|---|
 * | `threshold` | -50 … 0 dBFS | drempel |
 * | `ratio` | 0 … 4 | 1,5 / 2 / 3 / 4 / 6 :1 |
 * | `attack` | 0 / 1 | Fast (2 ms) of Slow (10 ms) |
 * | `release` | 0 … 5 | 100 / 400 / 800 ms / 1,5 s / A1 / A2 |
 * | `makeup` | 0 … +20 dB | uitgangsversterking |
 * | `color` | 0 … 2 | diodevervorming: 0 schoon, 2 dik |
...
class DiodeCompStream : public AudioStream {
public:
    DiodeCompStream() : AudioStream(2, inputQueue_) { comp_.Init(AUDIO_SAMPLE_RATE_EXACT); }

    mmb_dsp::DiodeComp& comp() { return comp_; }
...
private:
    ...
    mmb_dsp::DiodeComp comp_;
    volatile float gr_ = 0.0f;
};

class DiodeCompModule final : public AudioModule {
public:
    static constexpr const char* kTypeId = "tp_mmb_diode_comp";

    explicit DiodeCompModule(std::string_view id) : AudioModule(kTypeId, id) {}

    AudioPort outputPort(std::string_view portId) const override {
        auto* s = const_cast<DiodeCompStream*>(&stream_);
    ...
    void setControl(std::string_view controlId,
        ...
        auto& c = stream_.comp();
        if      (controlId == "threshold") c.set_threshold_db(asFloat(-20.0f));
        else if (controlId == "ratio") c.set_ratio(static_cast<int>(asFloat(1.0f) + 0.5f));
        else if (controlId == "attack") c.set_attack(static_cast<int>(asFloat(1.0f) + 0.5f));
    ...
    static void registerFactory() {
        ...
        reg.register_(kTypeId,
            [](std::string_view id) -> std::unique_ptr<mb::runtime::Module> {
                return std::make_unique<DiodeCompModule>(id);
            });
    }

private:
    mutable DiodeCompStream stream_;
};

#pragma once
/**
 * @file TremoloModule.h
 * @brief Tremolo-pedaal, stereo (`tp_mmb_tremolo`).
 *
 * mmb_dsp::Tremolo: modes Amp (bias), Opto (fotocel), Harmonic (brownface:
 * laag/hoog in tegenfase) en Pan (auto-panner). Zonder rechterkabel krijgt
 * R hetzelfde als L.
 *
 * Poorten: in_l/in_r (in = L) → out_l/out_r (out = L); CV rate, depth (ook
 * `*_cv`). Controls: rate (Hz), depth, wave (0 sin/1 tri/2 sqr), mode
 * (0..3), shape, level.
 */
...
class TremoloStream : public AudioStream {
public:
    TremoloStream() : AudioStream(2, inputQueue_) { tr_.Init(AUDIO_SAMPLE_RATE_EXACT); }
    mmb_dsp::Tremolo& tr() { return tr_; }
...
class TremoloModule final : public AudioModule {
public:
    static constexpr const char* kTypeId = "tp_mmb_tremolo";
    explicit TremoloModule(std::string_view id) : AudioModule(kTypeId, id) {}

    AudioPort outputPort(std::string_view portId) const override {
        auto* s = const_cast<TremoloStream*>(&stream_);
    ...
    void writeCvPort(std::string_view portId, float value) override {
        auto& t = stream_.tr();
        if      (cvPortIs(portId, "rate"))  t.set_rate(value);
        else if (cvPortIs(portId, "depth")) t.set_depth(value);
    }
    ...
    void setControl(std::string_view controlId, mb::runtime::ControlValue value) override {
        ...
        auto& t = stream_.tr();
        if      (controlId == "rate")  t.set_rate(asFloat(4.5f));
        else if (controlId == "depth") t.set_depth(asFloat(0.6f));
        else if (controlId == "wave")  t.set_wave(static_cast<int>(asFloat(0.0f) + 0.5f));
        else if (controlId == "mode")  t.set_mode(static_cast<int>(asFloat(0.0f) + 0.5f));
        else if (controlId == "shape") t.set_shape(asFloat(0.0f));
        else if (controlId == "level") t.set_level(asFloat(1.0f));
    }
    ...
    static void registerFactory() {
        ...
        reg.register_(kTypeId, [](std::string_view id) -> std::unique_ptr<mb::runtime::Module> {
            return std::make_unique<TremoloModule>(id); });
    }

private:
    mutable TremoloStream stream_;
};

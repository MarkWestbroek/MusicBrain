// ADR 0009 / ADR 0010 / ADR 0004 \u2014 16-bit pitch breakout impl.
...
void CvOut16::tick() {
    if (!sink_) return;
    // Wire format identical to CvOut12; bit-depth difference is applied
    // on the board (16-bit DAC vs 12-bit DAC). Brain stays normalised.
...
void CvOut16::registerFactory() {
    auto& reg = Registry::global();
    if (reg.has(kTypeId)) return;
    reg.register_(kTypeId, [](std::string_view instanceId) -> std::unique_ptr<Module> {
        return std::make_unique<CvOut16>(instanceId);
    });
}

namespace {
const int kCvOut16AutoRegister = [] { CvOut16::registerFactory(); return 0; }();
}

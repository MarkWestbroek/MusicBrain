...
8. ✅ First concrete firmware runtime modules + tests. *(done — `runtime/Envelope.h` (abstract), `runtime/Ahdsr.{h,cpp}`, `runtime/Lfo.{h,cpp}` all self-register against `Registry::global()`. AHDSR adds `Curve { Linear, Exponential, Logarithmic }` matching the panel switch; LFO mirrors Tone.LFO plus panel extensions S&H, `bipolar`, and `Run { Always, Gated, OneShot }`. All timing in ticks at `kCvTickRateHz = 1000`, no Arduino-isms, runs on host for CTest. 61/61 tests green, including: AHDSR curve midpoint (Lin ≈ 0.5, Exp ≈ 0.25, Log ≈ 0.75) and LFO sine peak/trough, bipolar/unipolar, depth scaling, Gated freeze, OneShot single-cycle halt, S&H stability within a cycle, reset. Per-method Doxygen-style docstrings added to every header in `runtime/` so the firmware tree reads as the spec for new collaborators.)*
9. ✅ CV-breakout scaffolding. *(done — `runtime/CvBreakout.h` (abstract, host-testable via injectable `BreakoutSink`), `runtime/CvOut12.{h,cpp}` (8-ch 12-bit modulation), `runtime/CvOut16.{h,cpp}` (16-bit pitch — wire-format identical to CvOut12, board-side scaling differs), `runtime/GateOut.{h,cpp}` (N-channel digital, dedup on repeated state). All three self-register; address rebinding (`addr_case`, `addr_first_slot`) marks every slot dirty so the next tick resends. 67/67 tests green. Concrete board firmware (SPI sink implementation, mux/S&H timing, calibration tables) is per-board hardware work outside the core library and lands later. See ADR 0010 for how voice-stamp expansion drives per-voice breakout channels.)*

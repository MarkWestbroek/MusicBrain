...
| FW-12 | 2 | 📝 | **MPE (Expressive E Osmose).** Verkenning in `doc/plans/mpe.md`: stap 1 druk-poorten `press`/`pressK` (`0xD0`/`0xA0`, ook zonder MPE nuttig), stap 2 `mpe`-schakelaar met kanaal-per-stem, per-kanaal bend/slide (`slideK`), `bendRange` tot 96, stap 3 Osmose eraan (via de pc; geen USB-host in de firmware). fw 0.5.69 heeft de bend al per stem in `voicePitchV` klaargezet (`bendPitch`). |

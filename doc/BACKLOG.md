...
| ED-RV-9 | 3 | ⏳ | **Unison/spread + sim voice-allocatie (idee uit brondump #6).** Eén toets → N stemmen met detune-spread (unison) i.p.v. N losse noten; plus een volwaardige polyfone voice-allocatie in de browser-sim (nu speelt alleen de master-stem `in1`, zie SIM-LIMITATION). Grootste item; nog niet ingepland — los oppakken. Hangt samen met ED-CG-3 (multi-module sim-nodes) en een echte poly-sim. |

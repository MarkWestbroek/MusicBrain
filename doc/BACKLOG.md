...
| ED-SM-7 | 2 | ⏳ | **Sample-analyse tegen echt materiaal.** Getest op kunstmatige takes (9/9) en op een Elements-take (`make-test-bank.mjs`). Bevestigd: bij inharmonisch materiaal (Elements geometry 0,42, klokken, klankschalen) zit vrije YIN er octaven naast — daarom de velden *verwachte noten* + *stemreferentie* en `refinePitchNear()`, dat rond een bekende noot hermeet. Nog open: vleugel, tubular bells, klankschalen met echte microfoons. |
| ED-SM-9 | 2 | ⏳ | **Loop-detectie op inharmonisch materiaal.** De periode-gesynchroniseerde zoeker werkt op harmonische klanken (kwaliteit 0,98 op de testtake). Bij klokken/schalen bestaat er geen periode; daar valt hij terug op grof rasteren met lange loops. Beoordelen op echt materiaal en eventueel een lange crossfade forceren. |
...
| FW-10 | 1 | 🟡 | **Hertrigger-flank per stem.** Zowel voice-stealing als een nieuwe mono-noot zet `gate_[idx]` laag en binnen dezelfde aanroep weer hoog; de CvGraph bemonstert op 1 kHz en ziet die flank dus nooit — de envelope slaat niet opnieuw aan. Dezelfde klasse fout als de gate-collaps in de browser-sim (daar opgelost: gate laag, aanslag één blok later). Nodig: een retrigger-puls per stem die de CvGraph één tick laag houdt. Raakt de CvGraph, dus met oren erbij doen — zie `doc/todo-2026-09-20-teensy-aan-de-kabel.md`. |
| FW-12 | 2 | 🟡 | **MPE (Expressive E Osmose).** Verkenning in `doc/plans/mpe.md`. **Stap 1 ✅ (fw 0.5.70):** `press`/`pressK` (`0xD0`/`0xA0`) en `rel`/`relK` (release-velocity) door alle vier de paden, gemeten op de Teensy. Stap 2 `mpe`-schakelaar met kanaal-per-stem, per-kanaal bend/slide (`slideK`), `bendRange` tot 96, stap 3 Osmose eraan (via de pc; geen USB-host in de firmware). fw 0.5.69 heeft de bend al per stem in `voicePitchV` klaargezet (`bendPitch`). |
| FW-11 | 2 | ✅ | **Note-priority (PRIO).** `MidiInModule::monoWinner()` kiest last/low/high; mono én unison gebruiken hem. Hoorde eerder bij FW-1 als "accepted by the editor but not yet acted on". Trok meteen een ouder probleem recht: de monofone tak liep via de VoiceAllocator, die alleen klinkende noten kent, dus bij het loslaten van de bovenste toets viel de gate in plaats van terug te zakken naar een nog ingedrukte toets. Regels overgenomen uit Mutable Yarns (`note_by_priority`) en Surge (`releaseNotePostHoldCheck`). Core-tests `midiin_mono_priority_high_ignores_lower_keys`, `..._low_follows_the_lowest_key`, `midiin_unison_follows_the_priority_too`; de bestaande `midiin_mono_noteon_raises_gate_and_sets_pitch` legde het oude gedrag vast en is bijgewerkt. |
...
| FW-FX-4 | 2 | ✅ | **Effectenbatch 2026-09-25 (fw 0.5.71):** stereo tape echo met cross-feedback, vintage digitale echo (12-bit, modulatie, stereo), BBD-chorus/flanger (mono → stereo), ringmodulator (clean/diode, externe carrier) en analoge octaver (OC-2-flip-flop, CV op de niveaus). Kernels in `mmb_dsp`, wasm-spiegels, panelen, Solo ▾-combinaties; op de Teensy gemeten. **Batch 2 (fw 0.5.72):** harmonizer (twee stemmen, pitch-synchroon venster, V/Oct), plaat/veergalm (Dattorro + dispersieve veren), tremolo (amp/opto/harmonic/pan), stereo phaser. **Batch 3 (fw 0.5.73/0.5.78):** vibe (univibe-familie), rotary (Leslie-stijl, instelbare snelheden), shimmer, korrelstand in de harmonizer. Open ideeën: gated reverb. |
| FW-13 | 1 | 🟡 | **Stap 1+2 ✅ (fw 0.5.74–0.5.77):** hergebruik per type, buffers los tijdens parkeren, editor stuurt alle knoppen mee, STK-OOM per instrument, echo's naar PSRAM — tien pushes achter elkaar klinken, heap stabiel. **Open, stap 3:** modules écht uit de AudioStream-lijst halen en vrijgeven (en onRetire/onReuse voor Clouds/Rings/Elements/Plaits/sampler). Oorspronkelijk: **Retired-pool lekt: tweede config-push met STK = stilte.** `ProjectRuntime::applyConfig` zet verdwenen instanties in een pool die nooit vrijkomt (AudioStream-lifetime); na één re-push met nieuwe id's is `heapFree` 0 en blijft de nieuwe STK-stem stil (`retired 20`). De editor-optimizer houdt id's stabiel, maar elke seed/recept-push met nieuwe id's lekt. Fix-richting: instanties echt vrijgeven na een quiesce (disconnect, één audioblok wachten, dan delete buiten de ISR), of een vaste pool per type hergebruiken. Gezien 2026-09-25 bij het meten van de effectenbatch. |
| FW-FX-3 | 3 | 🔨 | **Emulaties van beroemde compressors** (plan: [`doc/plans/vintage-compressors.md`](plans/vintage-compressors.md); FET, Opto, VCA-bus, Vari-mu, Program EQ en Diodebrug gebouwd in fw 0.5.58–0.5.63; nog open: inregelen op het oor). EQ-spoor: [`doc/plans/vintage-eq.md`](plans/vintage-eq.md), Console EQ (fw 0.5.64) en Para EQ (fw 0.5.65) klaar; Air/Tilt open, bv. een FET-type (1176: snel, agressief, all-buttons), opto (LA-2A: traag, programma-afhankelijk), VCA-bus (SSL: strak, lijm) en variable-mu (Fairchild: buizen). Als modi of losse modules naast `tp_mmb_comp`. De kern in `mmb_dsp`, zodat de sim meedoet. Let op: een compressor ná een module kan wat die module al hard op ±1 afknipte niet herstellen (16 bits tussen modules), daarom zit de limiter van de sampler (fw 0.5.57, `mmb_dsp/limiter.h`) in de sampler zelf. |

...
| Release | Datum | Kern |
|---|---|---|
| editor 0.8.4 | 2026-09-07 | **Opnameknop in de simulator.** Browsergeluid in een bestand krijgen vroeg op de Mac om BlackHole plus een apparaat met meerdere uitvoerkanalen, en dan een DAW ernaast: een omweg langs het luidsprekerpad, met twee klokken die uit elkaar lopen. **⏺ Opname** naast Start/Stop tapt nu de master-som af en levert bij Stop een 24-bits WAV met patchnaam en tijdstempel. (1) `sim/wavRecorder.ts` — `encodeWav()` (16/24-bits PCM en 32-bits float, met `fact`-chunk waar de spec dat wil) plus `MasterRecorder`, die de blokken van een AudioWorklet verzamelt. Bewust geen `MediaRecorder` (Opus is geen bron voor een samplerbank) en geen `ScriptProcessorNode` (draait op de hoofdthread, dus elke hapering in React wordt een gat in de opname). (2) `public/rec/tap-worklet.js` — kopieert, buffert tot 4096 frames, post als transferable. (3) `AudioEngine.recorderTap()` met een `recordBus` die met opzet een `build()` overleeft en niet in `dispose()` wordt opgeruimd: anders viel de opname stil zodra je tijdens het opnemen een kabel verlegde. (4) De afloopregel toont piek in dBFS en `⚠ overstuurd` — een zachte opname merk je anders pas als de bankimport er int16 van maakt en je drie bits kwijt bent. **Onderweg gerepareerd:** `new AudioWorkletNode(ctx.rawContext, …)` faalt met *"parameter 1 is not of type 'BaseAudioContext'"*, want Tone's `rawContext` kan de schil van standardized-audio-context zijn; het moet via `ctx.createAudioWorkletNode()`, zoals `WasmModule` al deed. Tests (node): WAV-kop en kanaalvolgorde teruggelezen met een eigen parser, klemmen bij +1,5 in plaats van omklappen, en de blokadministratie van de worklet onder een schil — 5120 frames erin, dezelfde oplopende reeks eruit, ook over de blokgrens heen. De audioketen zelf is in de browser met de hand nagelopen, niet geautomatiseerd. Zie [browser-instrumenten.md §9](browser-instrumenten.md#9-de-opnameknop). |
